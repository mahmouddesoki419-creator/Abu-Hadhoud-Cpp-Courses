...
void printMonthCalender(short Month , short Year)
{
    ...
    short dayOrder = dayorderNumber(1 , Month , Year);// number day sun = 0 , mon = 1 ect..

    for(short i = 0; i < dayOrder; i++)
    {
        cout << setw(6) << ",";// نحدد مسافات اول يوم بالتقويم الاول
    }

    for(short D = 1; D <= OrderNumberDaysInMonth; D++)
    {
        cout << left << setw(6) << D;// نطبع الارقام

        // == dayOrder++ و في الشرط نلغي ++
        if(++dayOrder % 7 == 0)// sun = 0 + 1 // 
        {
            cout << endl;
        }
    }
