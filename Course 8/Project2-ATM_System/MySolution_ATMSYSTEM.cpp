...
bool isLoginValid(string AccountNumber , string PinCode ,  string FileName)
{
...
void showQuickWithdrawScreen()
{
    ...
    for(stClient& Client : vClients)
    {
        if(Client.AccountNumber == AccountNumberClient)
        {
            do 
            {
...
{
    if(isLoginValid(AccountNumber , PinCode, ClientsFileName))
        return true;
    else 
        return false;
}
...
void Login()
{
    ...
    do 
    {
        ...
        cin >> AccountNumberClient;
        cout << "Enter PinCode? ";
        cin >> PinCode;

        LoginFaild = !loadClientInfo(AccountNumberClient , PinCode);
