...
struct stQuizz
{
    stQuestion QuestionList[100];//عملنا 100 خانه من الستراكشر السوال 
    short NumberOfQuestions;
    enQuestionsLevel SelectedLevel;
    enOperationsType OpType;
    short NumberOfWrongAnswers = 0;
    short NumberOfRightAnswers = 0;
    bool isPass = false;// here better than the enum and enter pass , Notpass
};
...
enOperationsType GetRandomOperationType()//make that function becouse we wanted to clean code and عمل حساب لو نطور البرنامج مستقبلا وقرايه عمليات اخري 
{
...
stQuestion GenrateQuestion(enQuestionsLevel QuestionLevel , enOperationsType OpType )// paremetars = +-*/نوع ادخال ليفل و العمليه 
{
    ...
    {
        OpType = GetRandomOperationType();// عكس الليفل بيكون ثابت المستويات ونادر التعديل فيه عشان كدا سوينا فانكشن 
    }
    Question.Op = OpType;
...
string GetOpTypeSymbol(enOperationsType OpType)
{
    ...
    case enOperationsType::Div:
        return "/";
    default:
        return "Mix";// هنا مش ايوصلها نه يطبعها بسبب شرط توليد قبل كده خلاها ميوصلش الراندوم فانكشن خاص ب اوه بي
        //compailer satisfaction and defensive Programming 
        //"خطط للسيناريو المثالي، واستعد للسيناريو الكارثي."
    }
}
